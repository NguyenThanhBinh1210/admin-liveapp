...
export default function TopupRequestsPage() {
  ...
  const fetchTopupRequests = async () => {
    try {
      ...
      const response = await getTopupRequestsAPI(params);
      ...
      setError(err.message || 'Failed to load topup requests');
    } finally {
      setLoading(false);
    }
  };
  ...
  const EditableStatusCell = ({ transaction, onStatusUpdate }) => {
    ...
    const handleApprove = async () => {
      try {
        setIsUpdating(true);
        await updateTopupRequestStatusAPI(transaction._id, 'approved', 'Đã duyệt yêu cầu nạp tiền');
    ...
    const handleReject = async () => {
      try {
        setIsUpdating(true);
        await updateTopupRequestStatusAPI(transaction._id, 'rejected', 'Đã từ chối yêu cầu nạp tiền');
  ...
  if (error) {
    return (
      <MainCard title="Top-up Requests Management" content={false}>
        <Box sx={{ p: 3 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            <Card sx={{ bgcolor: 'primary.lighter' }}>
              <CardContent sx={{ p: 2 }}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <WalletOutlined style={{ fontSize: '24px', color: '#1976d2' }} />
                  <Box>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ bgcolor: 'success.lighter' }}>
              <CardContent sx={{ p: 2 }}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <DollarOutlined style={{ fontSize: '24px', color: '#2e7d32' }} />
                  <Box>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ bgcolor: 'warning.lighter' }}>
              <CardContent sx={{ p: 2 }}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <WalletOutlined style={{ fontSize: '24px', color: '#ed6c02' }} />
                  <Box>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ bgcolor: 'info.lighter' }}>
              <CardContent sx={{ p: 2 }}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <WalletOutlined style={{ fontSize: '24px', color: '#0288d1' }} />
                  <Box>
      ...
      {!loading && (
        <>
          <TableContainer component={Paper} sx={{ boxShadow: 'none' }}>
            <Table sx={{ minWidth: 650 }} aria-label="topup requests table">
              <TableHead>
                <TableRow>
